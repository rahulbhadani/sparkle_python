...
for i, Line in enumerate(Lines):
    ...
    real_time_factor.append('Simulation {}, RTF '.format(i) + splits[splits.index('f') + 1])
...
for j in range(0, len(speed[0])):
    ...
    for k in range(0, len(speed)):
        ...
        t0 = speed[k][0][np.sqrt(speed[k][0]['linear.x']**2 + speed[k][0]['linear.y']**2) > 0].iloc[0]['Time']
        ax[j].scatter(x= speed[k][j]['Time'] - t0, y = np.sqrt(speed[k][j]['linear.x']**2 + speed[k][j]['linear.y']**2),  s = s_i, label = 'Factor = {}'.format(real_time_factor[k]))
...
def vel_calc_rms(df_2dlist, overlap_plots = False):
    """
    Argument
    ---------
    ...
    for ii in range(0, len(df_2dlist)):
        for jj in range(0, len(df_2dlist)):
            ...
            df1['Message'] = np.sqrt(speed[ii][0]['linear.x'].iloc[:-25]**2 + speed[ii][0]['linear.y'].iloc[:-25]**2) # remove last 25 points when sim starts breaking down
            df2['Time'] = speed[jj][0]['Time'].iloc[:-25] - speed[jj][0]['Time'].iloc[0]
            df2['Message'] = np.sqrt(speed[jj][0]['linear.x'].iloc[:-25]**2 + speed[jj][0]['linear.y'].iloc[:-25]**2) # remove last 25 points when sim starts breaking down
            df1new, df2new = strymread.ts_sync(df1, df2, rate ='first', method = 'nearest')      
            shift = strymread.time_shift(df1new, df2new, correlation_threshold=0.85)
            ...
            for vehicle  in range(0,len(df_2dlist[0])):
                ...
                df1['Message'] = np.sqrt(df_2dlist[ii][vehicle]['linear.x'].iloc[:-25]**2 + df_2dlist[ii][vehicle]['linear.y'].iloc[:-25]**2)# remove last 25 points when sim starts breaking down
                df2['Time'] = df_2dlist[jj][vehicle]['Time'].iloc[:-25] - df_2dlist[jj][vehicle]['Time'].iloc[0]
                df2['Message'] = np.sqrt(df_2dlist[jj][vehicle]['linear.x'].iloc[:-25]**2 + df_2dlist[jj][vehicle]['linear.y'].iloc[:-25]**2) # remove last 25 points when sim starts breaking down
...
def plot_rmsheatmap(df_2dlist, rms_matrix, max_car, data_name = 'Velocity'):
    ...
    n_rows = int(np.ceil(np.sqrt(max_car)))
    fig, ax = bagpy.create_fig(nrows = n_rows, ncols = n_rows)
    fig2, ax2 = plt.subplots(n_rows, n_rows)
    fig2.set_figheight(n_rows*9.5)
    fig2.set_figwidth(n_rows*8)
    ax2 = ax2.ravel()

    for vehicle in range(0,len(df_2dlist[0])):
        cmap= sns.cubehelix_palette()
        g = sns.heatmap(rms_matrix[vehicle], annot=True, ax = ax2[vehicle], linewidths=.5, xticklabels = real_time_factor, yticklabels=real_time_factor,  square=True,  annot_kws={"size": 6*n_rows}, cbar_kws={"shrink": 0.60/(n_rows/4)}, cmap=cmap, vmax=np.max(rms_matrix)*1.05, vmin = 0.0, fmt='.2f')
        g.invert_yaxis()
        ax2[vehicle].set_title('{} RMS Error b/w\nsimulations for vehicle {}'.format(data_name, vehicle), fontsize = 7*n_rows)
        g.set_xticklabels(g.get_xmajorticklabels(), fontsize = 7*n_rows, rotation = 90)
        g.set_yticklabels(g.get_ymajorticklabels(), fontsize = 7*n_rows, rotation = 45)
        cbar = ax2[vehicle].collections[0].colorbar
        cbar.ax.tick_params(labelsize=8*n_rows)

    while(vehicle+1 < n_rows*n_rows):
        vehicle = vehicle  + 1
        ax2[vehicle].axis('off')
...
rms_matrix_speed  = vel_calc_rms(speed, overlap_plots = True)
%pylab inline
